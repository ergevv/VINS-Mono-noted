...
$$
\mathbf{r}_a(\mathcal{X}_a) \approx
...
$$
\text{NonlinearFactorGraph + Values} \rightarrow
\text{线性化} \rightarrow
\text{GaussianFactorGraph} \rightarrow
\text{变量消元} \rightarrow
\text{求解增量} \rightarrow
\text{retract 更新变量}
$$
...
$$
\mathbf{r}_j(\mathcal{X}_j) \approx
...
$$
\text{给 GTSAM 全历史图} \Rightarrow
\text{优化全历史变量}
$$

$$
\text{给 GTSAM 当前滑窗图} \Rightarrow
\text{优化当前滑窗变量}
$$
...
$$
\text{GTSAM fixed-lag smoothing} \approx
\text{固定时间长度窗口}
$$
